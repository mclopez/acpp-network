...
public:    
    ...
    void set_events(decltype(EPOLLOUT) events) {
        ...
        if (epoll_ctl(io_->fd(), EPOLL_CTL_ADD|EPOLL_CTL_MOD, fd_, &sd) == -1) {
            log_error("epoll_ctl 2");
        }

    }

    void set_events_once(decltype(EPOLLOUT) events) {
        // struct kevent ev_set = {0};
        ...
        if (epoll_ctl(io_->fd(), EPOLL_CTL_ADD, fd_, &sd) == -1) {
...
public:
    ...
    void cancel() {
        ...
        // if (epoll_ctl(epollfd_, EPOLL_CTL_DEL, timer_id_, nullptr) == -1) {
        //     log_error("epoll_ctl");
        // }
...
struct io_context_pimpl {
    ...
    io_context_pimpl() : run(false), epollfd(-1) {
    ...
    void wait_for_input() {
        ...
        while (run) {
            ...
            for (int i = 0; i < nev; i++) {
                auto data = (socket_base_pimpl *)events[i].data.ptr;
                    ...
                    if (data->listening_) {
                        ...
                        if (new_fd == -1) {
                            ...
                            if (data->callbacks_.on_accepted) {
                                ...
                                new_socket.pimpl_->set_events(EPOLLIN);
...
void io_context::add_socket(async_socket_base& as) {
    ...
    as.pimpl_->set_events_once(EPOLLOUT);
